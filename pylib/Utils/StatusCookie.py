...
class StatusCookie:
	...
	def mkEmailDir(self, jobname):
		"""Make the email_dir string.  Something like:
           ...
           RETURNS:
               email_dir string.
		"""
		email_dir = "/nfs/dist/dmp/amp/update/" + self.getMne( jobname ) + "/" + self.getEnv( jobname ) + "/" + jobname + "/email"
		return email_dir
	##################################################################################
	#	Enddef
	##################################################################################
	...
	def writeStatus(self, type="notice", host="none", app="none", status="ok", msg=""):
		...
		try:
			...
			self.logIt( "pylib.Utils.CookieStatus.writeStatus(): Unable to open " + statusFile + " for append. => " + str( inst.errno ) + ":" + str( inst.strerror ) + "\n" )
			self.error_msg = "Unable to open " + statusFile + " for append. => " + str( inst.errno ) + ":" + str( inst.strerror )
			return False
		#Endtry
