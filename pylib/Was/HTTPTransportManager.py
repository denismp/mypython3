#!/usr/bin/env jython
######################################################################################
##	HTTPTransportManager.py
##
##	Python module for HTTPTransportManager attributes.
##
######################################################################################
##
...
class HTTPTransportManager( AttributeUtils ):
	"""
    HTTPTransportManager class that contains HTTPTransport methods for the WebContainer.transport.
	"""
	...
	def __init__(
		...
		self.logger					= logger
		AttributeUtils.__init__( self, configService, scope, type='HTTPTransport', logger=self.logger )
		self.logMySelf()
		self.validate()
	...
	def testFunc( self ):
		...
		for configObject in self.configObjects:
			...
			########################################################
			pArgs = array( ['transports'], java.lang.String )
			#pArgs = None
...
#########################################################################
def main():
	myLogger	= MyLogger( LOGFILE="/tmp/HTTPTransportManager.log", STDOUT=True, DEBUG=True )
	#myLogger	= MyLogger( LOGFILE="/tmp/HTTPTransportManager.log", STDOUT=True, DEBUG=False )
	...
	templateListManager	= TemplateListManager( configService, type="HTTPTransport", logger=myLogger )
	myHTTPTransportManager	= HTTPTransportManager( adminObject, configService, templateListManager, scope=myscope, logger=myLogger)

	for mylist in myHTTPTransportManager.attributesList:
		myHTTPTransportManager.deepLogOfAttributes( mylist )
	#Endfor
	for mylist in myHTTPTransportManager.attributesList:
		myHTTPTransportManager.deepPrintOfAttributes( mylist )
	#Endfor
	try:
		fileName = "/tmp/denis.txt"
		FH = open( fileName, "w" )
		for mylist in myHTTPTransportManager.attributesList:
			myHTTPTransportManager.deepWriteOfAttributes( mylist, FH )
		#Endfor
		FH.close()
	except Exception, e:
		myLogger.logIt( "main(): " + str( e ) + "\n" )
	...
	myHTTPTransportManager.writeAttributes( "/nfs/home4/dmpapp/appd4ec/tmp/HTTPTransportManager.tsv" )

	myHTTPTransportManager.testFunc()

	#rc = myHTTPTransportManager.modifyStreamRedirect( 
	#						currentFileName='${SERVER_LOG_ROOT}/SystemOut.log',
	#						fileName='${SERVER_LOG_ROOT}/SystemOut.log',
	#						rolloverType='BOTH',
	#						maxNumberOfBackupFiles=5,
	...
	#						suppressStackTrace=False
	#						)
	#if rc: myHTTPTransportManager.saveSession( False )

	configService.closeMe()
	myHTTPTransportManager.closeMe()
	adminObject.closeMe()
