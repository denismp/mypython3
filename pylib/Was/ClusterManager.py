...
class ClusterManager( AttributeUtils ):
	"""ClusterManager class that contains Cluster MBean methods."""
	...
	def __init__(
		...
		AttributeUtils.__init__( self, self.configService, scope, type='Cluster', logger=self.logger )
		self.status				= None
		self.message			= None
		self.props				= None
		self.appName			= None
...
#########################################################################
def main():
	myLogger	= MyLogger( LOGFILE="/tmp/ClusterManager.log", STDOUT=True, DEBUG=True )
	...
	scope = "Cell=" + str( mycell ) + ":Cluster=" + str( mycluster ) + ":type=Cluster"
	attributeName = 'Server'
	attributeType = 'Server'
	attributeList = AttributeList()

	configService 	= ConfigService( adminClient=myclient, logger=myLogger )
	myClusterManager 	= ClusterManager(adminObject, scope, configService, logger=myLogger)
	
	myClusterManager.closeMe()
	adminObject.closeMe()
	##################################################################################
	#Enddef
	##################################################################################
