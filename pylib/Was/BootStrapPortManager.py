...
##
##	Python module for BootStrapPortManager attributes.
##
######################################################################################
##
...
class BootStrapPortManager( AttributeUtils ):
	"""
    BootStrapPortManager class that contains BOOTSTRAP_ADDRESS methods for the NameServer.
	"""
	...
	def modifyBootStrapAddress( 
		...
		for configObject in self.configObjects:
			self.debug( __name__ + ".modifyBootStrapAddress(): configObject=" + str( configObject ) + "\n" )
			self.debug( __name__ + ".modifyBootStrapAddress(): configObject type=" + str( type( configObject ) ) + "\n" )
			...
			endPointObjectName		= self.configService.configServiceHelper.getAttributeValue( propertyAttributeList, 'BOOTSTRAP_ADDRESS' )
			endPointAttributeList	= self.configService.getAttributes( self.configService.session, endPointObjectName, None, False )

			self.debug( __name__ + ".modifyBootStrapAddress(): BEFORE endPointAttributeList=" + str( endPointAttributeList ) + "\n" )
			self.debug( __name__ + ".modifyBootStrapAddress(): BEFORE endPointAttributeList type=" + str( type( endPointAttributeList ) ) + "\n" )
			#######################################################
			...
			self.debug( __name__ + ".modifyBootStrapAddress(): AFTER endPointAttributeList=" + str( endPointAttributeList ) + "\n" )
...
#########################################################################
def main():
	myLogger	= MyLogger( LOGFILE="/tmp/BootStrapPortManager.log", STDOUT=True, DEBUG=True )
	#myLogger	= MyLogger( LOGFILE="/tmp/BootStrapPortManager.log", STDOUT=True, DEBUG=False )
	...
	myBootStrapPortManager	= BootStrapPortManager( adminObject, configService, templateListManager, scope=myscope, logger=myLogger)

	for mylist in myBootStrapPortManager.attributesList:
		myBootStrapPortManager.deepLogOfAttributes( mylist )
	#Endfor
	for mylist in myBootStrapPortManager.attributesList:
		myBootStrapPortManager.deepPrintOfAttributes( mylist )
	#Endfor
	try:
		fileName = "/tmp/denis.txt"
		FH = open( fileName, "w" )
		for mylist in myBootStrapPortManager.attributesList:
			myBootStrapPortManager.deepWriteOfAttributes( mylist, FH )
		#Endfor
		FH.close()
	except Exception, e:
		myLogger.logIt( "main(): " + str( e ) + "\n" )
	...
	myBootStrapPortManager.writeAttributes( "/nfs/home4/dmpapp/appd4ec/tmp/BootStrapPortManager.tsv" )

	myBootStrapPortManager.testFunc()

	
	rc = myBootStrapPortManager.modifyBootStrapAddress( 
							hostName='dilabvirt31-v5',
							port='2815'
							)
	...
	myBootStrapPortManager.testFunc()
	if rc: myBootStrapPortManager.saveSession( False )

	configService.closeMe()
	myBootStrapPortManager.closeMe()
	adminObject.closeMe()
