...
##    template.py
##    Python script to:
##		be a template for a main python program
##################################################################################
##
##    MODIFICATION HISTORY:
##    DATE        WHOM                DESCRIPTION
##    02/24/2016  Denis M. Putnam        Created.
##################################################################################
...
from pylib.Tasks.FileTask import FileTask #@UnusedWildImport
...
def usage():
    ...
    usage_string = "template.py\n" + \
        "\t[-l, --logfile]  -- log file(optional: default is " + str(CONFIG['logfile']) + ")\n" + \
        "\t[-s, --stdout]   -- stdout on.\n" + \
        "\t[-d, --debug]    -- debug on.\n" + \
        "\t[-h, --help]     -- show usage.\n\n" + \
        "template.py --logfile /tmp/oxclient.log --stdout\n"
    print(usage_string)
#######################################################
#    Enddef
#######################################################
...
def getCmdOptions():
    ...
    my_opts = {}
    err = None
    required_opts = { 'help': True, 'debug': True, 'stdout': True, 'logfile': True }
    rc = 1

    try:
        opts, args = getopt.getopt(sys.argv[1:], "hdsl:", ["help", "debug", "stdout", "number=", "host=", "port=", "logfile="]) #@UnusedVariable
