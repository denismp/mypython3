...
def usage():
    ...
    usage_string = "gen_giblets.py\n" + \
                   "\t[-j, --jobname]         -- Something like QAMP_UAT0000000_090504130000i(required: default is " + \
                   CONFIG['jobname'] + ")\n" + \
                   "\t[-a, --ampid]           -- ID from AMP(required: default is " + CONFIG['ampid'] + ")\n" + \
                   "\t[-t, --tag_version]     -- General XML tag version to use(optional: default is " + CONFIG[
                       'tag_version'] + ")\n" + \
                   "\t[-m, --max_was_version] -- max WAS XML tag version to use for component defs(optional: default is " + \
                   CONFIG['max_was_version'] + ")\n" + \
                   "\t[--lib_version]         -- LIB XML tag version to use for component defs(optional: default is " + \
                   CONFIG['lib_version'] + ")\n" + \
                   "\t[-g, --generic_version] -- Generic XML tag version to use for component defs(optional: default is " + \
                   CONFIG['generic_version'] + ")\n" + \
                   "\t[-q, --quasi_version]   -- quasi WAS XML tag version to use for component defs(optional: default is " + \
                   CONFIG['quasi_version'] + ")\n" + \
...
def getCmdOptions():
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], "hdsj:a:t:m:g:q:l:",
                                   ["help", "debug", "stdout", "jobname=", "ampid=", "tag_version=", "max_was_version=",
                                    "lib_version=", "generic_version=", "quasi_version=", "logfile="])
    ...
    for k, v in list(required_opts.items()):
...
def doWork():
    ...
    if (myObject):
        ...
        jilCommand = "/nfs/dist/dmp/amp/" + myBin + "bin/AMP_MASTER_deploy_application -jobid " + CONFIG[
            'jobname'] + " -ampid " + CONFIG['ampid']
        rVal = myObject.writeAutosysInsertJil(CONFIG['work_dir'], hostName, CONFIG['jobname'], CONFIG['ampid'],
                                              jilCommand)
        logIt("doWork(): writeAutosysInsertJil() returned " + str(rVal) + "\n")
        if (not rVal): rc = 1
        rVal = myObject.writeAutosysUpdateJil(CONFIG['work_dir'], hostName, CONFIG['jobname'], CONFIG['ampid'],
                                              jilCommand)
...
def mkDirs():
    ...
    if (not os.access(cookie.getEmailDir(), os.F_OK)):
        try:
            os.makedirs(cookie.getEmailDir(), 0o777)
        except OSError:
